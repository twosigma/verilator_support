...
$split->edit_file
    (# The filename to be edited
     filename=>"$split->{outdir}/altsyncram_body.v",
     # Callback subroutine that takes file contents as a string
     # and returns the new file contents
     cb=>sub {
	 my $wholefile = shift;
	 ...
	 # The %mg here means to match multiple lines (you can put
	 # \n in the regexp), and to do it globally
#$wholefile =~ s%(.*\<tri[01]\> .*)%//$1 -- removed tristate%g;
	 $wholefile =~ s%(i_good_to_write_.2 = i_good_to_write_.*)%// $1 -- cut useless loop%g;
	 return $wholefile;
     });
...
$split->edit_file
    (# The filename to be edited
	 ...
	 # \n in the regexp), and to do it globally
#$wholefile =~ s%(.*\<tri[01]\> .*)%//$1 -- removed tristate%g;
	 $wholefile =~ s%endmodule%initial assert (family_hardcopyii != 1) else begin \$display("ERROR: Hardcopy II not supported"); \$stop; end // -- sorry, Hardcopy II\nendmodule%g;
	 return $wholefile;
     });
...
$split->edit_file
    (# The filename to be edited
     ...
     cb=>sub {
	 my $wholefile = shift;
     # Remove useless warnings about invalid address
     $wholefile =~ s%(.*Address pointed at.*)\n(.*)%//vsplitmodule: $1\n//vsplitmodule: $2%mg;
	 return $wholefile;
     });
